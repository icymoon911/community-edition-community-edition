/**
 * MainController - Primary view controller for the main tab panel.
 *
 * Group management operations are delegated to Rambox.view.main.GroupController
 * (a singleton helper) so that this controller focuses on tab lifecycle,
 * service CRUD, search, DND, and lock functionality.
 */
Ext.define('Rambox.view.main.MainController', {
    ...
    ,initialize: function(tabPanel) {
        ...
        if (reorderer !== undefined) {
            var names = reorderer.container.getLayout().names;
            reorderer.dd.dim = names.width;
            reorderer.dd.startAttr = names.beforeX;
            reorderer.dd.endAttr = names.afterX;
        }

        // Apply persisted group collapse state on startup
        Rambox.view.main.GroupController.applyGroupCollapseState();
    }
    ...
    /**
     * Focuses the webview every time the user changes tabs (enables autofocus).
     */
    ,onTabChange: function(tabPanel, newTab, oldTab) {
        localStorage.setItem('last_active_service', newTab.id);

        if (newTab.id === 'ramboxTab') {
            document.title = Rambox.app.getTotalNotifications() > 0
                ? 'Rambox (' + Rambox.app.getTotalNotifications() + ')'
                : 'Rambox';
            return;
        }

        if (!newTab.record.get('enabled')) return;
        ...
        // Update window title to include the active tab name
        document.title = Rambox.app.getTotalNotifications() > 0
            ? 'Rambox (' + Rambox.app.getTotalNotifications() + ') - ' + newTab.record.get('name')
            : 'Rambox - ' + newTab.record.get('name');
    }
    ...
    ,removeServiceFn: function(serviceId, total, actual, callback) {
        ...
        if (config.default_service === rec.get('id')) {
            ipc.send('setConfig', Ext.apply(config, { default_service: 'ramboxTab' }));
        }

        function clearData(webview, tab) {
            var currentWebView = require('electron').remote.webContents.fromId(
                webview.getWebContentsId()
            );

            currentWebView.clearHistory();
            currentWebView.session.flushStorageData();
            currentWebView.session.clearCache().then(function() {
                currentWebView.session.clearStorageData().then(function() {
                    currentWebView.session.cookies.flushStore().then(function() {
        ...
        if (btn) {
            Ext.Msg.confirm(locale['app.window[12]'], locale['app.window[14]'], function(btnId) {
                if (btnId === 'yes') {
                    ...
                    Ext.Msg.wait('Please wait until we clear all.', 'Removing...');
                    var count = Ext.getStore('Services').getCount();
                    var i = 1;
    ...
    // ──────────────────────────────────────────────
    // Search / filter (for the service list panel)
    // ──────────────────────────────────────────────
        ...
        if (!Ext.isEmpty(newValue) && newValue.length > 0) {
            ...
            Ext.getStore('ServicesList').getFilters().replaceAll({
                fn: function(record) {
                    ...
                    return record.get('name').toLowerCase().indexOf(newValue.toLowerCase()) > -1;
                }
            });
        } else {
    ...
    ,dontDisturb: function(btn, e, called) {
        ...
        Ext.getCmp('mainTabBar').getEl().toggleClass('dontdisturb');
    ...
    // ──────────────────────────────────────────────
    // Group management – delegated to GroupController
    // ──────────────────────────────────────────────

    ,toggleGroupCollapse: function(groupName) {
        Rambox.view.main.GroupController.toggleGroupCollapse(groupName);
    }

    ,expandGroup: function(groupName) {
        Rambox.view.main.GroupController.expandGroup(groupName);
    }

    ,renameGroup: function(oldName, newName) {
        Rambox.view.main.GroupController.renameGroup(oldName, newName);
    }

    ,changeGroupColor: function(groupName, color) {
        Rambox.view.main.GroupController.changeGroupColor(groupName, color);
    }

    ,muteGroupServices: function(groupName, muted) {
        Rambox.view.main.GroupController.muteGroupServices(groupName, muted);
    }

    ,deleteGroup: function(groupName) {
        Rambox.view.main.GroupController.deleteGroup(groupName);
    }

    ,openGroupManager: function() {
        Rambox.view.main.GroupController.openGroupManager();
    }

    ,rebuildTabBar: function() {
        Rambox.view.main.GroupController.rebuildTabBar();
    }
});
